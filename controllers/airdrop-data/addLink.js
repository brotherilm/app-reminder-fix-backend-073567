...
export async function addLink(req, res) {
  try {
    ...
    let { airdropId, label, url } = req.body;
...
export async function editLink(req, res) {
  try {
    ...
    let { airdropId, index, label, url } = req.body;
    ...
    const updateQuery = {};
    if (label !== undefined) {
      updateQuery["additionalAirdrop.$.links." + index + ".label"] =
        sanitizeInput(label);
    }
    if (url !== undefined) {
      updateQuery["additionalAirdrop.$.links." + index + ".url"] = url;
    }
...
export async function deleteLink(req, res) {
  try {
    ...
    const { airdropId, index } = req.body;
    ...
    const result = await collection.updateOne(
      ...
      {
        $unset: {
          [`additionalAirdrop.$.links.${index}`]: 1,
        },
      }
    );

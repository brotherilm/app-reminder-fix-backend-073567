...
export async function resetTimerAirdrop(req, res) {
  try {
    ...
    const { airdropId, countdownMinute } = req.body;

    const countdown = Date.now() + countdownMinute * 60 * 1000;
    ...
    const result = await collection.updateOne(
      ...
      {
        $set: {
          "additionalAirdrop.$.countdown": countdown,
        },
      }
    );
